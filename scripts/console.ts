...
async function askForWallet(): Promise<Wallet> {
  ...
  const wallet = ethers.Wallet.fromMnemonic(accounts.mnemonic, accounts.path + `/${accountNumber - 1}`);
...
function askForMaxPriorityFeePerGas(feeData: FeeData): BigNumber | undefined {
  const defaultPriorityFee = feeData.maxPriorityFeePerGas === null ? BigNumber.from(0) : feeData.maxPriorityFeePerGas;
...
async function trackDeployment<T extends Contract>(fn: () => Promise<T>, name: string = `Contract`): Promise<T> {
  for (;;) {
    try {
      ...
      console.log(`${name} address: ${explorerUrl(net.chainId, UrlType.ADDRESS, contract.address)}`);
      console.log(`${name} transaction: ${explorerUrl(net.chainId, UrlType.TX, contract.deployTransaction.hash)}`);
      ...
      const update = askYesNo(`Update 'deployments.json' with new ${name} address ${contract.address}?`);
      if (update) {
