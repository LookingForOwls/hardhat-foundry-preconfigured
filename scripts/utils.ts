...
export async function submitTxWait(tx: Promise<ContractTransaction>, txName = `transaction`): Promise<ContractReceipt> {
...
export async function submitTxFail(tx: Promise<ContractTransaction>, expectedCause?: string): Promise<void> {
...
export async function expectTxFail<T>(tx: Promise<T>, expectedCause?: string): Promise<void> {
  try {
    ...
    if (expectedCause) {
      ...
      // error cleaning
      let cause = error.message.replace(`VM Exception while processing transaction: reverted with reason string `, ``);
      // custom error specific
      cause = cause.replace(`VM Exception while processing transaction: reverted with custom error `, ``);
      // custom error specific, e.g. 'MsgNeedsAddr()' error to check for just 'MsgNeedsAddr'
      cause = cause.replace(`()`, ``);
      expect(cause).to.equal(`'` + expectedCause + `'`, `tx failed as expected, but unexpected reason string`);
    }
    return;
  }
...
export function parseEvent(receipt: TransactionReceipt, contractInterface: Interface): utils.LogDescription[] {
